...
class OrderController extends Controller
{
    ...
    public function index($id)
    {
        ...
        return view('order.detail', compact('barang'));
    }
    ...
    public function order(Request $request, $id)
    {
        ...
        //cek keranjang & pembayaran yang terbaaru
        $keranjang_2 = Keranjang::where('id_users', Auth::user()->id)->where('status', 0)->first();


        //cek data dari table pembayaran yang terbaru
    ...
    {
        $keranjang = Keranjang::where('id_users', Auth::user()->id)->where('status', 0)->first();
        $pembayarans = [];//untuk mendeklarasikan variable keranjang kedalam array
        if (!empty($keranjang)) {
            $pembayarans = Pembayaran::where('id_keranjangs', $keranjang->id)->get();
        }

        return view('order.keranjang', compact('keranjang', 'pembayarans' ));
    ...
    public function checkout()
    {
        ...
        return redirect('keranjang')->with('status', 'Checkout');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect('keranjang')->with('status', 'Berhasil Dihapus');
    }
}
